...
class Loader(object):
    ...
    def set_value(self, key, value, use_scope=True):
        ...
        if isinstance(data, dict):
            if value == ObjDict() and isinstance(data.get(k), dict):
                pass
            else:
                data[k] = value
    ...
    def load_key(self, key, value):
        if self.current_scope.is_freeform:
            self.set_value(
                self.current_scope.index, ObjDict([
    ...
    def load_scope(self, brace, flags, scope_key):
        if scope_key == '' and len(self.stack) > 1:
            ...
            if old_scope.is_freeform and new_scope.is_nested:
                self.set_value(
                    old_scope.index, ObjDict([
                        ('type', scope_key),
                        ('value',  ObjDict() if brace == '{' else [])
                    ])
                )
            else:
                self.set_value(
                  scope_key, ObjDict() if brace == '{' else [],
